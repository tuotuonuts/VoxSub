...
- D 模式进入时在同一 worker 预热快速 OCR 与当前翻译器。Medium/Document 档对变化画面
  先使用内置 Small 返回最多 24 行的快速结果；画面稳定且实际 GPU 可用时，再用用户所选
  模型（含方向分类）覆盖纠偏。相邻行按屏幕从上到下作为上下文批量翻译，批量输出必须
  保持一一对应；格式异常回退逐行，不允许把译文错配到坐标框。
...
- 生成式本地 ASR 与当前云 STT 保持整段终句能力；智能上下文额外挂载内置 Zipformer 草稿旁路，共享 VAD 后的音频窗口但不替代最终识别器。旁路只负责可丢弃的 partial，加载或运行失败必须回落到终句路径，不得中断录音。
- 不得用高频全量重推理伪装流式识别；草稿旁路固定使用轻量 Zipformer、最多 2 路 beam，选中的生成式/云模型只在原句界运行一次。
- `asr_live_draft_enabled` 默认开启且只控制草稿识别/临时翻译；关闭不得连带关闭智能断句、上下文处理、终句纠偏或语气词清理。非上下文调优方式保持原有 partial 行为。
...
### 组件清单（M7 验收依据）
1. **主窗**：编辑式左右分栏（左=四个平级模式 A/B/C/D + 常驻语言方向 + 状态灯；右=A/B/C 字幕工作区或 D OCR 工作区）；底部胶囊 CTA 仅在 A/B/C 显示，普通同传为“开始/停止”，录音同传为“开始/暂停/继续”，另提供“结束并保存”
