...
- OCR 翻译缓存会随翻译器配置切换失效；生产导入故障不能以 stub 模拟成功；仅在
  所属工作线程全部退出后才能释放云客户端、翻译器等运行时资源。
- 常规 PR / push 门禁在 Windows 上从带哈希的 `requirements.lock` 同步依赖，再执行
  编译检查和完整测试；NPU 工作流继续作为需真实硬件的手动验证。
...
- 智能断句、保守纠偏与语气词清理封装在 Qt 无关的 `contextual_text.py`；
  Pipeline 只拥有有界上下文队列和线程生命周期，旧调优模式不会进入该阶段。
- 上下文最长等待从首个待定片段开始计时，新片段不能续期；流式与生成式
  ASR 共享同一未完句判断，但仍保留各自原有解码方式。
...
- 文本纠偏必须保留原始文本和可审计修改记录，且不得使用无证据的自由改写；
  所有语义等待必须有不可续期的硬截止时间。
...
- 输出缓存必须把会影响结果的配置纳入有效期；配置切换时必须失效或分区，不能复用
  旧模型、旧端点或旧密钥产生的结果。
- fallback 只能用于显式测试注入或用户可见的降级路径，不能把核心组件加载失败伪装成
  正常运行。
- 组件拥有的工作线程未退出前，禁止关闭或替换组件持有的原生/网络资源。
