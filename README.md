...
脚本优先使用已安装的 `uv`，否则自动下载并校验便携 uv；没有系统 Python 时由 uv 自动下载并管理 Python 3.11。若同步时发现 `.venv` 被占用或权限异常，脚本会先将它改名备份，再用当前电脑重建后重试。依赖或运行时网络失败时，窗口会显示易懂的原因，详细输出保存到 `%LOCALAPPDATA%\VoxSub\diagnostics\source-run\`；基础功能会在依赖已满足时继续启动，网络恢复后再次双击即可重试。脚本不会复制其他电脑的 `.venv`，也不会删除模型、配置或用户数据。
