...
- 内置诊断与实时日志：诊断页可开启 20 分钟自动结束的临时调试会话；日志、诊断报告和会话导出均使用应用内保存窗口并在后台写入
...
- 中日韩英翻译：语言方向支持中文、英文、日文、韩文之间互译，以及自动源语言 → 固定目标语言。内置 OPUS 极速档仍仅支持中英；日语/韩语请在设置中选择 Hy-MT2 质量档或云翻译。质量档使用模型广场中标注“33 种语言”的 Hy-MT2 模型，未安装模型时请先下载对应模型。
...
要求：Windows 10/11。源码运行不会把模型或配置写进 Git 工作树；Python、uv 和 Git 缺失时，一键启动脚本会在用户目录自动准备便携版本。
...
给朋友使用时，先获得一个带 `.git` 目录的源码副本（首次可由熟悉 Git 的人完成克隆或使用 GitHub Desktop），然后双击项目根目录的 `更新并启动测试版.bat`。以后每次双击同一个文件即可自动更新源码、准备本机 Python/uv、创建独立 `.venv`、同步 `requirements.lock` 并启动 `run_app.py`。Python、uv 或 Git 不存在时，脚本会下载固定版本到 `%LOCALAPPDATA%\VoxSub\bootstrap`，无需管理员权限或手动配置。首次真正使用质量翻译时，应用还会先自动修复缺失的 CPU/Vulkan/OpenVINO 运行时并执行真实探针；修复失败才按 GPU → CPU 顺序降级，不会伪报 NPU。脚本会清理 `PYTHONPATH` / `PYTHONHOME`，测试环境自动使用 `testing` 标记。GitHub 下载的 ZIP 不含 `.git`，因此不能作为后续 `git pull` 更新的源码副本。
...
脚本优先使用已安装的 `uv`，否则自动下载并校验便携 uv；没有系统 Python 时由 uv 自动下载并管理 Python 3.11。若同步时发现 `.venv` 被占用或权限异常，脚本会先将它改名备份，再用当前电脑重建后重试。CPU/Vulkan/OpenVINO 运行时下载会校验远端固定的文件大小、ZIP 文件头和 SHA256，失败自动重试 3 次；如果代理或网关返回 HTML、下载截断或缓存损坏，窗口和 `%LOCALAPPDATA%\VoxSub\diagnostics\source-run\` 日志会记录实际大小、摘要前缀和文件头，便于定位网络问题。基础功能会在运行时网络失败时继续启动，网络恢复后再次双击即可重试。脚本不会复制其他电脑的 `.venv`，也不会删除模型、配置或用户数据。
...
本地日志位于 `%LOCALAPPDATA%\VoxSub\logs\voxsub.log`，也可在应用“诊断 → 日志”页查看和导出。需要更详细现场时，在“诊断 → 日志”开启“诊断调试”；会话默认 20 分钟后自动结束，也可手动关闭。手动上传会话期间只提交该会话标记的日志，并附带行数与隐私过滤统计。“诊断 → 自检”页的“导出报告 (txt)”会生成当前最新自检结果；修复按钮只处理该结果中明确声明的目标。

配置 DSN 后，诊断页的“自检”会显示“发送到 Sentry”，用于上传当前最新自检报告和本地日志快照（隐私过滤后），不会重新执行自检；“日志”页也提供独立的“上传日志到 Sentry”，不需要先运行自检。为避免大日志把现场问题挤出附件，超过上传上限时会保留最新的完整日志行。上传完成后，日志页会显示来源和上传时间范围、原始/上传/省略行数、脱敏统计；同一份安全元数据也会写入 Sentry 附件头部。未配置或网络失败时不会影响应用。

“诊断 → 日志 → 清除本机日志”需要确认后才执行：它会清空 `%LOCALAPPDATA%\VoxSub\logs` 中的当前/轮转日志，以及 `%LOCALAPPDATA%\VoxSub\diagnostics` 中的启动日志；不会删除模型、配置、用户数据、Sentry 配置或已手动导出的报告。正在运行的 VoxSub 会继续写入一份新的空日志。
