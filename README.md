...
- 0.9.0-beta 候选新增与修复：修复安装包缺少 `rapidocr.main` 导致两种 OCR 均失败；框选前等待 Windows 完成隐藏主窗，消除应用虚影；支持上传图片、原位覆盖预览及译后图片导出；原图与译后图采用非 C 盘分离缓存，默认每类 15 张/0 无限；模型广场新增四档 OCR 模型并可直接切换。OCR 入口移到顶部，A/B/C 模式卡不再被挤压。
