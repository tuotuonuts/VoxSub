...
> **已知限制：Intel NPU 调度仍在开发中。** 当前版本可能识别到 Intel NPU 硬件，但尚不能保证模型实际运行在 NPU 上；推理可能回退到 GPU 或 CPU。请不要把本版本视为 NPU 加速已可用，相关问题正在继续排查。
...
- 大众硬件路由：独显 GPU → NPU → 核显 → CPU；GPU/核显降级已接入，Intel NPU 调度仍存在已知问题，当前可能回退到 GPU/CPU
...
模型广场不是模型仓库的全量镜像。目录只保留 VoxSub 已有运行时适配、许可证明确，并在相近资源档位仍有价值的模型：Fun-ASR-Nano、Qwen3-ASR、SenseVoice Small，以及 Hy-MT2 1.8B/7B 的 Q4/Q5/Q8 量化档。内置 Zipformer/OPUS 仅作为极低资源兜底。
