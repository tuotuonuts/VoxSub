...
当前源码候选版本：`0.9.0-beta`；GitHub 当前公开下载仍为 `0.7.2-beta`。实时 OCR 视觉与稳定性修复继续沿用同一候选版本，在用户验收前不会冒充已发布版本。
