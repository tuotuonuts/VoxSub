...
- 0.9.0-beta 候选新增与修复：修复安装包缺少 `rapidocr.main` 以及旧 PyInstaller 运行时残留遮蔽依赖导致两种 OCR 均失败；升级前只清理应用管理的运行时目录，保留 `Models` 与 `Cache`；框选前等待 Windows 完成隐藏主窗，消除应用虚影；支持上传图片、原位覆盖预览及译后图片导出；原图与译后图采用非 C 盘分离缓存，默认每类 15 张/0 无限；模型广场新增四档 OCR 模型并可直接切换。OCR 现在是主界面 D 平级模式，常驻显示翻译方向；进入模式即后台预热 DirectML OCR 和翻译模型，变化画面使用快速首帧、稳定画面使用所选质量模型纠偏。实时首轮限制为 20–24 个版面块和约 2.2–2.4K 字符，批量 JSON 异常时不再退回逐行请求；采集队列只保留最新画面并丢弃过期结果，页面切换不会被旧翻译任务拖住。实时覆盖层的 Windows 捕获排除已修正，不再每轮隐藏/显示；同一行碎片和连续正文会先合并成段落块，主正文列优先，已有目标语言界面不重复翻译；长译文按实际字体度量整块换行，覆盖框会根据可用内尺寸自适应字号，URL、标识符和无空格的中日韩文本也能任意字符换行，避免末尾内容被裁剪；失败项保留可读原文，GPU 失败时安全回退 CPU。安装升级现在为正常收尾保留 5 秒、强制关闭后再复核进程状态，且运行标记保持到进程真正终止；打包成品的真实退出握手为 2.23 秒，不再因退出竞态误报“无法关闭 VoxSub”。
