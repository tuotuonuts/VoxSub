...
当前源码候选版本：`0.8.0-beta`；GitHub 当前公开下载仍为 `0.7.2-beta`。`0.8.0-beta` 完成构建与验证前不会冒充已发布版本。
...
- OCR 屏幕翻译：截图模式可框选图片、网页或文档并查看带坐标的原文与译文；实时模式持续观察选定屏幕范围，只在画面明显变化时重新识别，并在原文位置覆盖显示译文。控制条支持暂停、临时显示原文、重选区域和结束
- 模型广场：按识别/翻译/语音朗读分类和质量排序，可下载、切换、卸载开源模型；根据本机 CPU、内存、GPU/显存标注“不推荐 / 较为推荐 / 推荐 / 满载”
...
安装包默认输出到项目上一级的 `Release` 目录；本项目开发工作区对应 `D:\OneDrive\app_dve\Release`。新安装默认把模型放在安装目录下的 `Models` 文件夹，并按用途分到 `stt`、`translate`、`vad`、`tts`；升级用户保留现有模型根目录，直到主动在设置中更改。模型属于用户数据，不会被安装包重复分发或在更新时删除。当前公开的 `0.7.2-beta` 安装包为 214,817,504 字节，SHA256 `313714AE3C9557B88EDBCEBBFCB768A15BBDD65915A5266E0B3EB1D82CAF2211`。本地 `0.8.0-beta` 候选安装包为 277,266,873 字节（264.42 MiB），SHA256 `04997F7A48B5EB878C4303F6BCCAEC5A72CDD82864131DC5E994FB03AE8E1365`；尚未创建 GitHub Release。
