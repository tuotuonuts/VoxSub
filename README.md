...
当前源码候选版本：`0.9.0-beta`；GitHub 当前公开下载仍为 `0.7.2-beta`。本次 OCR 性能修复按“先推送源码、再打包”流程重建同版本安装包，在用户验收前不会冒充已发布版本。
