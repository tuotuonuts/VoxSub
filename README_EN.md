...
Current source candidate: `0.8.0-beta`; the current public GitHub download remains `0.7.2-beta`. The candidate is not presented as released until its build and verification are complete.
...
- **OCR screen translation:** Screenshot mode selects an image, webpage, or document region and shows coordinate-aware source and translated text. Live Region mode watches a selected screen area, reruns OCR only after a meaningful visual change, and paints translations over the original text positions. Its control bar can pause, temporarily reveal the source, reselect, or stop.
