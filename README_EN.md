...
- **0.6.0-beta candidate feature:** Smart Context now keeps one replaceable current-sentence draft that updates near word cadence and can revise earlier text. Its translation follows the changing source and accepts only the newest request result; the row enters history only when the sentence is final. Streaming Zipformer supplies frequent partials, while generative local ASR and the current cloud STT API continue to update per complete acoustic segment to avoid repeated expensive inference and requests.
