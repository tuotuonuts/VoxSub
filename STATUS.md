...
- [~] v0.9.0-beta OCR 完善候选：除图片上传、覆盖导出、分离缓存和四档模型外，实时 OCR 已接入 DirectML GPU 并保留 CPU 安全回退；多行译文改为有界批量请求与逐行异常降级。1920×1080/30 行 Medium 真机从 CPU 热帧约 24 秒降至 DirectML 热帧约 0.45 秒；源码门禁 `294 passed / 8 skipped`。上一安装包生成于性能修复前，当前按“先推送、后打包”流程重建；用户验收前不发布 Release
