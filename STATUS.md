...
- [ ] v0.9.0-beta OCR 完善候选：修复打包内 RapidOCR 延迟导入；修复模式卡挤压与框选桌面虚影；新增图片上传、原位覆盖预览、译后图片导出、非 C 盘 originals/translated 分离缓存及 15/0 无限设置；模型广场新增四档可切换 OCR 模型；源码门禁 `286 passed / 8 skipped`，等待安装包与 SHA256
...
1. 完成 v0.9.0-beta 全量门禁、正式安装包、隔离启动、打包内 OCR 冒烟与图片缓存/导出验证；推送 GitHub，用户验证前不创建新 Release
