...
- [x] v0.9.0-beta OCR 完善候选：OCR 改为与 A/B/C 平级的 D 模式，共用常驻翻译方向；进入模式后台预热快速 OCR 与翻译器，变化画面先用 Small 返回最多 24 行，稳定画面在实际 GPU 可用时用所选质量模型纠偏；相邻 OCR 行按上下文有界批量翻译。源码门禁 `298 passed / 8 skipped`；成品 D 模式隔离启动保持运行，英→中方向生效，预热 2.09 秒且日志确认 `GPU · DirectML`。安装包 277,277,256 字节（264.43 MiB），SHA256 `79C9A348...F688766`；未签名，等待用户安装验收，不发布 Release
...
1. 用户安装验收 v0.9.0-beta：重点检查 A/B/C/D 四模式布局、D 模式左侧翻译方向、首次进入的预热提示、实时快速结果/稳定纠偏、截图/上传/覆盖导出和非 C 盘分离缓存；验收前不创建新 Release
