...
- [x] v0.9.0-beta OCR 完善候选：OCR 改为与 A/B/C 平级的 D 模式，共用常驻翻译方向；进入模式后台预热快速 OCR 与翻译器，变化画面使用 Small 快速结果，稳定画面在实际 GPU 可用时用所选质量模型纠偏。实时覆盖捕获排除已修正，不再周期隐藏闪烁；碎片/连续正文按段落块合并，主阅读列优先，目标语言界面过滤，长译文按度量整块换行，空结果保留旧画面重试。源码门禁 `304 passed / 8 skipped`；成品 D 模式隔离启动保持运行，英→中方向生效且日志确认 `GPU · DirectML`。继续沿用同版本候选，未签名，等待用户安装验收，不发布 Release
...
1. 用户安装验收 v0.9.0-beta：重点检查实时覆盖不闪烁、邮件/文档连续正文合并为大块且无裁切、主正文未被侧栏挤掉，以及 A/B/C/D 布局、翻译方向、快速结果/稳定纠偏、截图/上传/导出和非 C 盘分离缓存；验收前不创建新 Release
