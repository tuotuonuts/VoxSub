...
- [ ] v0.9.0-beta OCR 完善候选：修复打包内 RapidOCR 延迟导入、升级残留不完整 `brotlicffi` 遮蔽依赖、模式卡挤压与框选桌面虚影；新增图片上传、原位覆盖预览、译后图片导出、非 C 盘 originals/translated 分离缓存及 15/0 无限设置；模型广场新增四档可切换 OCR 模型；升级清理应用运行时但保留 `Models`/`Cache`；等待重新打包与安装升级验收
