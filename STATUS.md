...
- [x] v0.9.0-beta OCR 完善候选：除图片上传、覆盖导出、分离缓存和四档模型外，实时 OCR 已接入 DirectML GPU 并保留 CPU 安全回退；多行译文改为有界批量请求与逐行异常降级。1920×1080/30 行 Medium 真机从 CPU 热帧约 24 秒降至 DirectML 热帧约 0.45 秒；源码门禁 `294 passed / 8 skipped`，成品 OCR 冒烟退出码 0 且日志确认 DirectML。安装包 277,338,824 字节（264.49 MiB），SHA256 `9B6D5757...34ED6B`；未签名，等待用户安装验收，不发布 Release
