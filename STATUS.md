...
- [x] v0.9.0-beta OCR 完善候选：修复打包内 RapidOCR 延迟导入、升级残留不完整 `brotlicffi` 遮蔽依赖、模式卡挤压与框选桌面虚影；新增图片上传、原位覆盖预览、译后图片导出、非 C 盘 originals/translated 分离缓存及 15/0 无限设置；模型广场新增四档可切换 OCR 模型；升级清理应用运行时但保留 `Models`/`Cache`；门禁 `287 passed / 8 skipped`，成品 OCR 冒烟退出码 0；安装包 277,283,040 字节（264.44 MiB），SHA256 `7F5DB195...528DF7DC`；开发者自签名，等待用户安装验收，不发布 Release
...
1. 用户安装验收 v0.9.0-beta：重点检查 A/B/C 模式布局、截图/上传/实时区域 OCR、框选无主窗虚影、原图与译后图非 C 盘分离缓存、译后图片导出及 OCR 模型切换；验收前不创建新 Release
