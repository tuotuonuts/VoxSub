...
- [x] v0.9.0-beta OCR 完善候选：修复打包内 RapidOCR 延迟导入；修复模式卡挤压与框选桌面虚影；新增图片上传、原位覆盖预览、译后图片导出、非 C 盘 originals/translated 分离缓存及 15/0 无限设置；模型广场新增四档可切换 OCR 模型；门禁 `286 passed / 8 skipped`，成品 OCR 冒烟退出码 0；安装包 277,259,168 字节（264.41 MiB），SHA256 `D2D52B2...D70E9659`；开发者自签名，等待用户验证，不发布 Release
