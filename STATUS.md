...
| D 模式 OCR（截图框选 / 图片上传 / 实时区域 / 原位覆盖） | 工作区已实现；完整交互未走完 |
...
| 后端能力实测 | `tools/probe-backend.py`：19/20 PASS（1 项为预期的参数校验拒绝） |
